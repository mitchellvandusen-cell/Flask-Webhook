...
# ── GHL Calendar fetch (used from dashboard config tab) ──────────────────────

@demo_bp.route("/api/fetch-calendars", methods=["GET"])
def fetch_calendars():
    """Fetch calendars from GoHighLevel for the current user's location."""
    ...
    try:
        ...
        data = resp.json()
        calendars = [
            {"id": cal.get('id'), "name": cal.get('name', 'Unnamed Calendar')}
