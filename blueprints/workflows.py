...
PREBUILT_WORKFLOWS = [
    {
        ...
        "steps": [
            {"id": "s1", "step_type": "control", "step_subtype": "wait",
             "config": {"duration": 30, "unit": "seconds"}, "x": 400, "y": 240},
            {"id": "s2", "step_type": "action", "step_subtype": "send_sms",
             "config": {"message": "Hi {{firstName}}, this is {{operatorName}} with {{companyName}}. I saw you were looking into coverage options — I'd love to help you find the best fit. When's a good time to chat?"}, "x": 400, "y": 360},
            {"id": "s3", "step_type": "control", "step_subtype": "wait",
             "config": {"duration": 5, "unit": "minutes"}, "x": 400, "y": 480},
            {"id": "s4", "step_type": "condition", "step_subtype": "if_else",
             "config": {"conditions": [{"field": "responded", "operator": "responded_within", "value": "5"}], "logic": "and"}, "x": 400, "y": 600},
            {"id": "s5", "step_type": "action", "step_subtype": "ai_call",
             "config": {"ring_timeout": 30}, "x": 200, "y": 740},
            {"id": "s6", "step_type": "action", "step_subtype": "add_tag",
             "config": {"tag": "speed-to-lead-contacted"}, "x": 600, "y": 740},
    ...
    },
    {
        ...
        "steps": [
            {"id": "s1", "step_type": "condition", "step_subtype": "if_else",
             "config": {"conditions": [{"field": "temperature", "operator": "temperature_is", "value": "cold"}], "logic": "and"}, "x": 400, "y": 240},
            {"id": "s2", "step_type": "action", "step_subtype": "send_igb_message",
             "config": {"mode": "ai", "prompt_hint": "This is a cold lead we haven't heard from in over 30 days. Send a short, low-pressure check-in. Mention that rates have changed recently and offer an updated quote with no obligation."}, "x": 200, "y": 380},
            {"id": "s3", "step_type": "action", "step_subtype": "send_igb_message",
             "config": {"mode": "ai", "prompt_hint": "This is a warm lead we haven't spoken to recently. Re-engage naturally, reference past conversation if possible, and offer to provide an updated quote."}, "x": 600, "y": 380},
            {"id": "s4", "step_type": "action", "step_subtype": "add_tag",
             "config": {"tag": "re-engage-sent"}, "x": 400, "y": 540},
            {"id": "s5", "step_type": "control", "step_subtype": "wait",
             "config": {"duration": 2, "unit": "days"}, "x": 400, "y": 660},
            {"id": "s6", "step_type": "condition", "step_subtype": "if_else",
             "config": {"conditions": [{"field": "responded", "operator": "responded_within", "value": "2880"}], "logic": "and"}, "x": 400, "y": 780},
            {"id": "s7", "step_type": "action", "step_subtype": "add_tag",
             "config": {"tag": "re-engaged"}, "x": 600, "y": 920},
            {"id": "s8", "step_type": "action", "step_subtype": "send_igb_message",
             "config": {"mode": "ai", "prompt_hint": "This is a final follow-up for an aged lead who hasn't responded to our re-engagement. Send a friendly closing message — let them know the door is always open if they need help with coverage in the future."}, "x": 200, "y": 920},
    ...
    },
    {
        ...
        "steps": [
            {"id": "s1", "step_type": "condition", "step_subtype": "if_else",
             "config": {"conditions": [{"field": "tags", "operator": "has_tag", "value": "do-not-contact"}], "logic": "and"}, "x": 400, "y": 240},
            {"id": "s2", "step_type": "control", "step_subtype": "exit",
             "config": {}, "x": 200, "y": 380},
            {"id": "s3", "step_type": "condition", "step_subtype": "if_else",
             "config": {"conditions": [{"field": "temperature", "operator": "temperature_is", "value": "hot"}], "logic": "and"}, "x": 500, "y": 380},
            {"id": "s4", "step_type": "action", "step_subtype": "add_tag",
             "config": {"tag": "hot-lead-responded"}, "x": 300, "y": 540},
            {"id": "s5", "step_type": "action", "step_subtype": "ai_call",
             "config": {"ring_timeout": 30}, "x": 300, "y": 680},
            {"id": "s6", "step_type": "action", "step_subtype": "send_igb_message",
             "config": {"mode": "ai", "prompt_hint": "The lead just replied to our SMS. Continue the conversation naturally and try to book an appointment or gather more details about their coverage needs."}, "x": 650, "y": 540},
            {"id": "s7", "step_type": "control", "step_subtype": "exit",
             "config": {}, "x": 400, "y": 820},
        ],
        "connections": [
    ...
    },
    {
        ...
        "steps": [
            {"id": "s1", "step_type": "action", "step_subtype": "send_sms",
             "config": {"message": "Hey {{firstName}}, just checking in! I know life gets busy. If you're still thinking about coverage, I'd be happy to answer any questions. Just reply here anytime."}, "x": 400, "y": 240},
            {"id": "s2", "step_type": "action", "step_subtype": "add_tag",
             "config": {"tag": "re-engage-attempt"}, "x": 400, "y": 360},
            {"id": "s3", "step_type": "control", "step_subtype": "wait",
             "config": {"duration": 1, "unit": "days"}, "x": 400, "y": 480},
            {"id": "s4", "step_type": "condition", "step_subtype": "if_else",
             "config": {"conditions": [{"field": "responded", "operator": "responded_within", "value": "1440"}], "logic": "and"}, "x": 400, "y": 600},
            {"id": "s5", "step_type": "action", "step_subtype": "add_tag",
             "config": {"tag": "re-engaged"}, "x": 600, "y": 740},
            {"id": "s6", "step_type": "action", "step_subtype": "ai_call",
             "config": {"ring_timeout": 25}, "x": 200, "y": 740},
            {"id": "s7", "step_type": "control", "step_subtype": "wait",
             "config": {"duration": 3, "unit": "days"}, "x": 200, "y": 880},
            {"id": "s8", "step_type": "action", "step_subtype": "send_sms",
             "config": {"message": "Hi {{firstName}}, I wanted to reach out one last time. If you ever need help with insurance coverage, don't hesitate to reach out. Wishing you the best!"}, "x": 200, "y": 1020},
            {"id": "s9", "step_type": "action", "step_subtype": "add_tag",
             "config": {"tag": "nurture-complete"}, "x": 400, "y": 1160},
