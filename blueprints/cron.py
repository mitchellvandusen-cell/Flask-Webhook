...
# ── Support bot log cache refresh ────────────────────────────────────────────

@cron_bp.route("/api/cron/refresh-support-logs", methods=["GET", "POST"])
def api_refresh_support_logs():
    """
    Refresh the support_log_cache table with recent server logs and
    per-user error summaries. Run every 15 minutes.

    - Captures Railway deployment logs (if railway CLI available)
    - Captures per-user error summaries from webhook_logs
    - Deletes cached logs older than 7 days
    """
    if not _cron_authorized():
        return safe_jsonify({"error": "Unauthorized"}), 401
    ...
    try:
        conn = get_db_connection()
        if not conn:
            return safe_jsonify({"success": False, "error": "DB unavailable"}), 200

        cur = conn.cursor()

        # 1. Clean old logs (>7 days)
        cur.execute("DELETE FROM support_log_cache WHERE captured_at < NOW() - INTERVAL '7 days'")
        cleaned = cur.rowcount

        # 2. Capture Railway logs via CLI (if available)
        for service_name, source_key in [("Flask-Webhook", "railway_flask"), ("worker", "railway_worker")]:
            try:
                result = subprocess.run(
                    ["railway", "logs", "--service", service_name, "--lines", "500"],
                    capture_output=True, text=True, timeout=30
                )
                if result.returncode == 0 and result.stdout.strip():
                    cur.execute("""
                        INSERT INTO support_log_cache (source, location_id, log_content)
                        VALUES (%s, NULL, %s)
                    """, (source_key, result.stdout[-50000:]))  # Cap at 50KB
                    captured += 1
            except (FileNotFoundError, subprocess.TimeoutExpired):
                # Railway CLI not installed or timeout — skip silently
                pass
            except Exception as e:
                logger.warning(f"Failed to capture {service_name} logs: {e}")

        # 3. Capture per-user error summaries (locations with errors in last 3 days)
        try:
            cur.execute("""
                SELECT location_id,
                       ...
                       STRING_AGG(
                           ...
                           LEFT(COALESCE(details::text, ''), 200),
                           E'\\n'
            ...
            for row in cur.fetchall():
                ...
                cur.execute("""
                    INSERT INTO support_log_cache (source, location_id, log_content)
                    VALUES ('user_errors', %s, %s)
                """, (loc_id, summary[-20000:]))  # Cap at 20KB
                captured += 1
        except Exception as e:
            logger.warning(f"Failed to capture user error summaries: {e}")

        conn.commit()
        cur.close()

        logger.info(f"Support log refresh: captured={captured}, cleaned={cleaned}")
        return safe_jsonify({"success": True, "captured": captured, "cleaned": cleaned})

    except Exception as e:
        logger.error(f"Cron refresh-support-logs crashed: {e}", exc_info=True)
        if conn:
            try:
                conn.rollback()
            except Exception:
