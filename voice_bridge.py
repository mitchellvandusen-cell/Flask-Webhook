...
# ──────────────────────────────────────────────────────────────
# TRAINING — Data transfer code for InsuranceGrokBot Training
# ──────────────────────────────────────────────────────────────

import secrets as _secrets
import hashlib as _hashlib

def _generate_training_code(location_id: str) -> str:
    """Generate a deterministic-prefix + random training transfer code.

    Format: igt_<location_hash8>_<random32>
    The location hash lets the Training platform quickly identify the account
    without exposing the raw location_id.
    """
    loc_hash = _hashlib.sha256(location_id.encode()).hexdigest()[:8]
    random_part = _secrets.token_urlsafe(24)
    return f"igt_{loc_hash}_{random_part}"


@voice_bp.route('/voice/training/generate-code', methods=['POST'])
@login_required
def training_generate_code():
    """Generate (or regenerate) a training transfer code for the current user.

    The code is stored in voice_config['training'] alongside metadata.
    The InsuranceGrokBot Training platform uses this code to pull recordings.
    """
    subscriber, vc, sub_sid = _get_current_subscriber_voice()
    if not subscriber:
        return jsonify({"error": "Subscriber not found"}), 404
    ...
    location_id = subscriber.get('location_id', '')
    code = _generate_training_code(location_id)
    ...
    logger.info(f"Training transfer code generated for {current_user.email}")
    return jsonify({
        "ok": True,
        "transfer_code": code,
...
@voice_bp.route('/voice/training/status', methods=['GET'])
@login_required
def training_status():
    """Return current training transfer code status."""
    subscriber, vc, sub_sid = _get_current_subscriber_voice()
    if not subscriber:
        return jsonify({"error": "Subscriber not found"}), 404

    training = (vc or {}).get('training', {})
    code = training.get('transfer_code', '')
    return jsonify({
        "has_code": bool(code),
        "transfer_code": code,
        "generated_at": training.get('generated_at', ''),
        "total_recordings": _count_user_recordings(subscriber.get('location_id', '')),
    })


@voice_bp.route('/voice/training/revoke-code', methods=['POST'])
@login_required
def training_revoke_code():
    """Revoke the current training transfer code."""
    subscriber, vc, sub_sid = _get_current_subscriber_voice()
    if not subscriber:
        return jsonify({"error": "Subscriber not found"}), 404

    vc = vc or {}
    vc.pop('training', None)
    _save_voice_config(current_user.email, vc)

    logger.info(f"Training transfer code revoked for {current_user.email}")
    return jsonify({"ok": True})


def _count_user_recordings(location_id: str) -> int:
    """Count call recordings available for a location_id."""
    conn = get_db_connection()
    if not conn:
        return 0
    try:
        ...
        cur.execute(
            ...
            "WHERE location_id = %s AND recording_url IS NOT NULL",
            (location_id,)
        )
        row = cur.fetchone()
        cur.close()
        ...
        logger.error(f"_count_user_recordings error: {e}")
        return 0
    finally:
        return_db_connection(conn)


@voice_bp.route('/voice/training/export', methods=['GET'])
def training_export_recordings():
    """Public API for InsuranceGrokBot Training platform to fetch recordings.

    Auth: Bearer <training_transfer_code> header.
    Returns paginated call recording metadata (URL, duration, transcript, etc.).
    """
    ...
    code = auth[7:].strip()
    if not code or not code.startswith('igt_'):
        return jsonify({"error": "Invalid transfer code"}), 401

    # Look up subscriber by training code
    conn = get_db_connection()
    if not conn:
    ...
    try:
        cur = conn.cursor()
        cur.execute(
            "SELECT location_id, email, voice_config FROM subscribers "
            "WHERE voice_config->'training'->>'transfer_code' = %s",
            (code,)
        )
        row = cur.fetchone()
        cur.close()
        ...
        if not row:
            return jsonify({"error": "Invalid or revoked transfer code"}), 401
