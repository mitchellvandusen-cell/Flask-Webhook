...
def send_sms_via_ghl(
    ...
    Returns:
        (True, None)         — SMS sent successfully
        (True, 'duplicate')  — Already sent recently (dedup)
        ...
        (False, 'safety')    — Blocked by safety filter
        (False, 'invalid')   — Invalid contact_id or missing token
    ...
    if conn:
        try:
            ...
            if cur.fetchone():
                ...
                return True, 'duplicate'  # Treat as success (already sent)
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            elif status in (401, 403):  # Auth issue — don't retry with same token
                logger.error(f"Auth failure ({status}) — aborting retries for token recovery")
                return False, 'auth'
