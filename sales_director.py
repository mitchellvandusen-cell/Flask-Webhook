...
def _build_objection_guidance(logic: LogicSignal, bot_settings: dict = None, objection_log: List[str] = None) -> str:
    ...
    if in_phase_3:
        ...
        header += (
            ...
            "PHASE 2 TOOLKIT (pick ONE approach per message — do not stack techniques):\n"
            "- CONSEQUENCE QUESTIONS (NEPQ/Miner): 'What happens if you keep pushing this "
            "down the road and something happens?' Force them to confront inaction cost.\n"
            "- COST OF INACTION (Gap Selling): Make the gap between their current state "
            "(unprotected) and future state (family secure) feel real and specific to THEIR life.\n"
            "- DESIRE FOR GAIN vs FEAR OF LOSS (Tracy): Warm leads respond to gain ('peace of "
            "mind starting this week'). Cool leads respond to loss ('one incident could change "
            "everything for your family'). Match the lead's temperature.\n"
            "- LOOPING (Belfort): If the objection keeps coming back, do not repeat the same "
            "argument. Loop back and rebuild whichever trust element is weakest: trust in YOU, "
            "trust in the PRODUCT, or trust in the COMPANY. Each loop attacks a different angle.\n\n"
        )
    else:
        header += (
            ...
            "TOOLKIT (pick ONE per message — this is TEXT, not a phone call. Be natural):\n"
            "- LABELING (Chris Voss): ONLY when the lead sounds emotional, frustrated, or defensive. "
            "'Seems like...' or 'Sounds like...' names their hidden feeling and defuses it. "
            "Do NOT use a label every message — over text it sounds robotic and formulaic.\n"
            "- CALIBRATED QUESTIONS (Voss): 'How?' or 'What?' questions give them the illusion "
            "of control. 'What would need to change for this to make sense?' beats 'Is now good?'\n"
            "- SELL THE APPOINTMENT, NOT THE PRODUCT (Brian Tracy): You are booking a "
            "10-minute call, not selling a policy. Never discuss product details over text.\n"
            "- ACCUSATION AUDIT (Voss): ONLY for hostile/annoyed leads. List the negative thing "
            "they are probably thinking before they say it. They will correct you and reveal the "
            "real objection. Use sparingly — once per conversation max.\n\n"
        )
    ...
    if obj == ObjectionType.NOT_INTERESTED:
        ...
        return header + (
            ...
            "TWO TECHNIQUES — pick the one that fits the conversation:\n\n"
            "TECHNIQUE 1: THE FORK (primary)\n"
            "Step 1: AGREE. 'Yeah totally fair' or 'No I get it.' Short. Casual. Zero resistance.\n"
            "Step 2: THE FORK. Split 'not interested' into two specific paths:\n"
            "'Was it more that the price was too high, or did you have a tough time getting approved?'\n"
            "Variations: 'Was it more the cost or was it a health thing?' / 'Was it more a budget "
            "thing or did you run into trouble qualifying?'\n"
            "Both paths lead back to a real conversation.\n\n"
            "TECHNIQUE 2: ACCUSATION AUDIT (Chris Voss — ONLY if they seem annoyed/hostile)\n"
            "List the negative thing they are probably thinking BEFORE they say it:\n"
            "'I know — you probably think I am just another insurance person trying to sell you "
            "something you do not need.'\n"
            "Their natural response is to correct you: 'No it is not that, it is just...' — and "
            "NOW they reveal the real objection. Use this ONCE per conversation max. Over text "
            "it can sound sarcastic if overused.\n\n"
            "CRITICAL: Pick ONE technique. Keep to 1-2 sentences total. Agree + fork OR "
            "accusation audit. Not both. Not a paragraph. This is a text message, not a pitch."
        )
    ...
    if obj == ObjectionType.SPOUSE_PARTNER:
        ...
        return header + (
            ...
            "STEP 2: The Hypothetical Removal (Sales Sniper) — isolate whether the spouse "
            "is the REAL objection or a shield:\n"
            "'If your [wife/husband] was here right now and totally on board with it, would "
            "getting the right coverage in place be something you would want to do?'\n"
            "If YES → the spouse is logistical, not a smokescreen. They genuinely want this.\n"
            "If they hesitate or say NO → the spouse is a shield. There is another real objection "
            "hiding behind it. Dig: 'What specifically would hold you back even if they were on board?'\n\n"
            "STEP 3: Tie the spouse to what THEY want (Miner):\n"
            "'How does your [wife/husband] feel about having stronger financial protection "
            "for the house and everything else if something happens to you?'\n"
            "The answer is always yes. Nobody says 'my spouse would not want us protected.'\n\n"
            "STEP 4: Position the call as HELPING their spouse conversation:\n"
            ...
            "A 10-minute call to gather information is not a purchase decision.\n\n"
            "REMEMBER: The spouse does not need to approve an appointment — they need to "
            "approve a purchase. Those are different things. 'This isn't signing anything — it is "
            "just getting the info so you two can make a smart decision together.'"
        )
    ...
    if obj == ObjectionType.PRICE_MONEY:
        ...
        return header + (
            ...
            "STEP 1: Hypothetical Removal (Sales Sniper + Miner) — isolate whether money is "
            "the REAL objection:\n"
            "'If money was not an issue at all, is this something you would want to have in "
            "place for your family?'\n"
            "If YES → they want it but think they cannot afford it. Redirect to the call where "
            "they learn real numbers (most people think insurance costs 10x what it does).\n"
            "If they hesitate → money is a smokescreen. There is another concern hiding. "
            "Dig: 'What else would hold you back even if the price was right?'\n\n"
            "STEP 2: Clarify what 'too expensive' actually means. Miner says always find out:\n"
            ...
            "One question: 'Is it that you genuinely cannot swing it right now, or is it more "
            "that you are not sure it is worth it?'\n\n"
            "STEP 3: Either way, redirect to the call: 'It takes 10 minutes to see what it "
            "would actually cost for your situation. Most people are surprised how affordable "
            "it is.'\n\n"
            "REMEMBER: You are not selling a policy. You are booking a 10-minute call. "
            "Price concerns about the APPOINTMENT do not exist — the call is free. "
            "If they are worried about insurance cost, the call IS the answer.\n\n"
    ...
    if obj == ObjectionType.ALREADY_COVERED:
        if in_phase_2:
            ...
            return header + (
                ...
                "GET TO 'THAT'S RIGHT' (Voss): Summarize what they told you about their "
                "coverage until they say 'yeah that is right' or 'exactly.' Once they confirm "
                "you understand their situation, they are open to hearing what is missing. "
                "Example: 'So you have [carrier] through work, it covers about [amount], and "
                "it has been in place for [time] — is that right?' When they confirm, THEN "
                "ask: 'And if you left that job tomorrow, what happens to it?' The gap reveals "
                "itself after agreement, not before.\n\n"
                "IF THEY GOT BURNED BEFORE (bad agent, bad experience, felt scammed):\n"
                "Sales Sniper 'Been Burned Before' reframe — do NOT dismiss their experience. "
                "Acknowledge it fully first: 'Yeah that is a rough experience. I get why you would "
                "be cautious.'\n"
                "Then: 'Has that bad experience stopped you from wanting to make sure your family "
                "is actually protected though? Or is it more about not wanting to deal with another "
                "bad agent?'\n"
                "If it is the agent → you are a different person. The call is 10 minutes and no "
                "pressure. Position yourself as the opposite of what burned them.\n"
                "If it is the product → find out what went wrong. Wrong product type? Overpaid? "
                "Coverage they did not need? The call is where you fix that."
            )
        return header + (
    ...
    if obj == ObjectionType.THINK_ABOUT_IT:
        if in_phase_2:
            return header + (
                ...
                "THREE REFRAMES to try (pick the one that fits the conversation):\n\n"
                "1. THE RISK REFRAME (Miner):\n"
                "'Which is more risky — spending 10 minutes on a call to see where you stand? "
                "Or keeping things the way they are and hoping nothing happens in the meantime?'\n\n"
                "2. THE 3/6/12 MONTH REFRAME (Sales Sniper — 'Fear of Unknown'):\n"
                "'What happens if 3 months or 6 months from now you are in this exact same spot — "
                "still thinking about it, still not sure? What changes?'\n"
                "This is powerful because the answer is NOTHING changes. They know it. Insurance "
                "does not get cheaper with time. Health does not improve with age. The only thing "
                "that changes is the cost goes up and the options go down.\n\n"
                "3. THE EMOTIONAL ARC CALLBACK:\n"
                "If they shared anything — kids, spouse, mortgage, health scare — use it: "
                "'You mentioned [person/thing]. Are they covered right now while you are "
                "thinking it over?'\n\n"
                "If they STILL stall after all of this, set the appointment anyway (Miner): "
                "'No problem, what is your timeframe on getting back to me in the next day "
                "or two?' Book a specific time. Do NOT accept 'I will call you back.' "
                "That means never.\n\n"
                "LAST-MINUTE WOBBLE (Sales Sniper): If all else fails:\n"
                "'You are going to have to do this eventually — whether it is now, next month, "
                "or next year. And each time you delay it just gets more expensive. The hardest "
                "part is deciding. After that it is just a 10-minute call.'"
            )
        return header + (
    ...
    if obj == ObjectionType.BUSY_TIMING:
        ...
        return header + (
            ...
            "GAP SELLING FLIP: Their busyness is actually the reason to act, not the reason to "
            "wait. Busy people have more at stake — more income, more dependents counting on "
            "them, more to lose. Do NOT say this directly (sounds preachy). But the subtext of "
            "your message should be: 'You are busy BECAUSE your family depends on you. That is "
            "exactly why 10 minutes matters.'"
        )
    ...
    return header + (
        ...
        "SMOKESCREEN CHECK (Sales Sniper): If you have seen this lead cycle through "
        "MULTIPLE different objection types in the OBJECTION_LOG (e.g., first it was money, "
        "then spouse, then busy, then think about it) — they are not objecting. They are "
        "smokescreening. None of the objections are real. They just do not want to say NO "
        "directly. In this case, give them permission to say no: 'Hey, it is totally fine if "
        "this is not for you. No hard feelings. I would just rather hear that than keep going "
        "back and forth.' Sometimes naming the smokescreen breaks it. Sometimes they say no "
        "and you move on. Either way, you stop wasting time.\n\n"
